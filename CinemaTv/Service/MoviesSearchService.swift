...
final class MoviesSearchService {
    ...
    class func loadMoviesSeach(
        page: String,
        movie: String,
        endpoint: MoviesEndpoint,
        complitionHandler: @escaping (Result<MoviesSearchModel, APIServiceError>)-> ()
    ) {
        ...
        let dataTask = session.dataTask(with: request) { (data: Data?, response: URLResponse?, error: Error?) in
            if error == nil {
                ...
                if response.statusCode == 200 {
                    ...
                    do {
                        let movies = try JSONDecoder().decode(MoviesSearchModel.self, from: data)
